...
class SystemTrayIcon(QSystemTrayIcon):
    def __init__(self, icon, handlers, parent=None):
        ...
        maxAllMicsAction = QAction("Apply to all mics", parent)
        maxAllMicsAction.setCheckable(True)
        maxAllMicsAction.setChecked(config["apply_all_mics"])
        maxAllMicsAction.triggered.connect(handlers["on_toggle_apply_all_mics"])
        menu.addAction(maxAllMicsAction)
